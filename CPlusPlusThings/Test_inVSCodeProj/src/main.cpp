...
/**
 *  created on 2021.8.9
 *  @ref:https://www.cnblogs.com/ittinybird/p/4830834.html
 * 
 *  atomic operation
 * 
 * */

// 多线程最主要的问题就是共享数据带来的问题。如果共享数据都是只读的，那么没问题，因为只读操作不会影响到数据，更不会涉及对数据的修改，
// 所以所有线程都会获得同样的数据。但是，当一个或多个线程要修改共享数据时，就会产生很多潜在的麻烦。
...
/*
    c++多线程程序中，每个线程都有一个线程栈，它们相互独立，因此在线程栈中的数据，是不会被其他线程影响到的。
    但是在内存的数据段中的数据，是可以在全局被访问到的。我们在上面这段代码中定义的sum变量正是位于数据段(&edata)中。
    
    在目前来看，我们期望最后程序退出的时候，打印出sum是 9999900000。但是结果却不尽人意，我们试着编译运行1000次（用脚本写个运行程序）：

    1000次运行中还是有315次得到了正确答案，有685次的结果是偏小的，却没有一次的结果是偏大的！那么问题出在哪里了？
    
    试着想象一下这样一个场景:你和朋友合租在一间房子里边，房子里面只有一间厨房，你们共用一个锅。有一天你准备做一道西红柿炒蛋，
    当你把西红柿放入锅中的时候，你的电话响了，你离开厨房去接电话。
    而这时候你的室友也要做饭，他要做一道红烧鱼，于是他把洗好的鱼放入了锅中煮，然后也离开了厨房
    （由于某种原因他不知道锅里还有你的食材，在程序中线程也不会知道其他线程对共享的数据做了什么）。
    当你回来的时候继续往里边放入鸡蛋，最后你得到的是一盘西红柿炒鸡蛋鱼。而你的室友回来厨房的时候他要的红烧鱼就会不见了。

    在上面的例子里，你和室友就代表着thread1和thread2线程，sum变量就是那个锅。多线程中共享数据的问题，就是上面场景中你们共用一口锅造成的问题。

    有一中可行的方案就是：你们做菜的步骤很短，短到什么程度呢，短到这个步骤不可被分割。例如你做的这道菜只有一个步骤，
    就是让食材（对应于下面提到的原子数据类型）碰一下锅（当然现实场景中基本没有这样的菜），这样你们的做菜过程就不会被其他室友打断、干扰，即使你们共同在使用一口锅。

    sum += i;操作被分割成了两条cpu指令，先是将i的值保存在eax寄存器中，然后将eax的值加上sum的值并保存在sum中。

    而在c++中原子操作就是这样的一种『小到不可分割的』操作。要使用原子操作我们需要引用c++11的一个新的头文件<atomic>。
      在这个头文件中定义了一个类模板struct atomic表示原子数据类型，在GNU的实现（/usr/include/c++/4.8.3/atomic）上如下：

    我们之前的sum变量是long类型的，对应的原子数据类型是std::atomic_long，下面我们就简单在开篇的代码中改动和添加：

*/

#include <atomic>                 // modified
 
std::atomic_long sum = {0L};  　　//  modified，注意不要写成『std::atomic_long sum = 0L』的形式，因为long类型是不可以隐式转换为std::atomic_long类型的。
